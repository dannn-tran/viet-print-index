...
def iter_source_items_from_veridian(
    ...
    catalogue_html = http.fetch_text(_catalogue_url_for(catalogue_url, a="cl", cl="CL1", sp=config.title_id, ai="1"))
    issues = sorted(_issues_from_catalogue_html(catalogue_html, config.title_id), key=lambda issue: issue.published_on)
    for issue in issues:
        if not config.from_date <= issue.published_on <= config.to_date:
            continue
        issue_html = http.fetch_text(_catalogue_url_for(catalogue_url, a="d", d=issue.oid))
        for page in _parse_pages(issue_html, issue.oid):
            yield DiscoveredSourceItem(
                kind="image",
                source_url=_page_image_url(config.image_server_url, page),
                issue_id=issue.oid,
...
def _catalogue_url_for(catalogue_url: str, **query: str) -> str:
    return catalogue_url + "?" + urllib.parse.urlencode(query)


def _page_image_url(image_server_url: str, page: Page) -> str:
...
def _parse_pages(issue_html: str, expected_issue_oid: str) -> list[Page]:
...
def _issues_from_catalogue_html(catalogue_html: str, title_id: str) -> list[Issue]:
    oids = dict.fromkeys(_ISSUE_RE.findall(html.unescape(catalogue_html)))
    return [
        issue
        for oid in oids
        if (issue := _issue_from_oid(oid, title_id=title_id)) is not None
    ]


def _issue_from_oid(oid: str, *, title_id: str) -> Issue | None:
